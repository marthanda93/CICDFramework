...
class GithubExecutor implements IGithubRegistry, IMissingObject, Serializable {
	...
	Boolean plainClone(Map appParam) {
		if(ContextRegistry.getContext().getShellExecutor().bashShellStatus("if [ -d ${_steps.globalPipelineSetting.gitReferencePath}/${appParam.url.split('/')[-1]} ]; then echo 'true'; else echo 'false'; fi")) {
			_steps.println "INFO: Reference is available!"
			if(ContextRegistry.getContext().getShellExecutor().bashShellStatus("[ \"\$(ls -A /tmp)\" ] && echo 'true' || echo 'false'")) {
