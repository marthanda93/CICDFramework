...
class GithubExecutor implements IGithubRegistry, IMissingObject, Serializable {
	...
	Boolean plainClone(Map appParam) {
		if(ContextRegistry.getContext().getShellExecutor().bashShell("if [ -d ${_steps.globalPipelineSetting.gitReferencePath}/${appParam.url.split('/')[-1]} ]; then echo 'true'; else echo 'false'; fi")) {
