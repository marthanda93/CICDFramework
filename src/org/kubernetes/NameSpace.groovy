...
class NameSpace implements IK8NameSpaceRegistry, IMissingObject, Serializable {
	...
	Boolean create(Map k8Param) {
		...
		_steps.println CommonUtilities.executeOnMaster("ls ${_steps.env.JENKINS_HOME}/workspace/${_steps.env.JOB_NAME}@libs/${_steps.env.getEnvironment().findAll { it.key =~ /^library.(.+).version$/ }.keySet()[0].split('\\.')[1]}")
